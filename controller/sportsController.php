<?php
     ...
     class sprotsController
     {
        function __construct()
        {
            $this->objconfig = new config();
            $this->objsm = new sprotsModel($this->objconfig);
        }
        // mvc handler request
        ...
        // page radirection
        public function pageRadirect($ur1)
        {
            header('Locatin:'.$ur1);
        }
        //  check Validation($sporttb)
        public function checkValidation($sporttb)
        { $noerror=true;
           // Validate category
           if(empty($sporttb->category)){
              $sporttb->category_msg = "Field is empty.";$noerror=false;
           } elseif(!filter_var($sporttb->category, FILTER_VALIDATE_REGEXP,
   array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
    $sporttb->category_msg = "Invalid entry.";$noerror=false;
     ...
     if(empty($sporttb->name)){
        ...
        } elseif(!filter_var($sporttb->name, FILTER_VALIDATE_REGEXP,
       array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
